...
class Main
{
	...
	static public function getJsText()
	{
		global $wpdb;
		// заголовок окна
		$title = __('Be the first to know', 'ms-plugins') . '!';
		// текст при успешной подписке
		$congratulations = __('An email has been sent to you with a link to confirm your email. Follow the link in the letter to receive the newsletter.', 'ms-plugins');
		$close = __('close', 'ms-plugins');
		...
		return json_encode(array(
			...
			'close' => $close,
			'placeholderemail' => __('Your e-mail', 'ms-plugins'),
			'subscribe' => __('Subscribe', 'ms-plugins'),
			'subscribeText' => __('Subscribe to news', 'ms-plugins'),
			'emptyresult' => __('Nothing found', 'ms-plugins'),
			'finded' => __('finded', 'ms-plugins'),
			'success' => __('Success', 'ms-plugins'),
			'enterpageid' => __('You must enter page id or find and choose page by title before save. Page id is required', 'ms-plugins'),
			'chooseone' => __('choose one', 'ms-plugins'),
			'comingsoon' => __('This functionality will be available in future releases.', 'ms-plugins'),
			'avt' => __('Free version'),
			'onsubscribemess1' => __('An email has been sent to', 'ms-plugins'),
			'onsubscribemess2' => __('Follow the link in email to subscribe to the newsletter.', 'ms-plugins'),
			'congratulate' => __("Congratulations! Subscription successful!", 'ms-plugins'),
			'unsubscribe' => __("You have successfully unsubscribed from the newsletter.", 'ms-plugins'),
			'error' => __('Error', 'ms-plugins'),
			'novisualeditorfound' => __('No visual editor found. Try to refresh the page. If this does not help, please contact the plugin support.', 'ms-plugins'),
			'entertext' => __('Enter the text of the letter', 'ms-plugins'),
			'whatactiontoperform' => __('What action to perform', 'ms-plugins'),
			'lettertosubscribers' => __('Letter to subscribers', 'ms-plugins'),
			'lettertoadministrator' => __('Sample letter to administrator', 'ms-plugins'),
			'paidversion' => __('The functionality is available in the paid version', 'ms-plugins') . ' (' . self::getOption('price') . ' RUB)',
			'getupgrade' => __('Get upgrade', 'ms-plugins'),
			'wrongvalue' => __('Check the correctness of the entered data', 'ms-plugins'),
			'duplicateshavebeenremoved' => __('duplicates have been removed', 'ms-plugins'),
			'beforeunload' => __('Leave the page? Changes made will not be saved.', 'ms-plugins'),
			'entertemplatename' => __('Enter template name', 'ms-plugins'),
			'yes' => __('Yes'),
			'no' => __('No'),
			'save' => __('Save'),
			'replacetemplate' => __('Template exist. Replace?', 'ms-plugins'),
			'replacecontent' => __('Replace content?', 'ms-plugins')
		));
	}
	...
	static public function getOptionPage()
	{
		...
		$param['smtp'] = Main::getOption(array('smtp_email', 'smtp_password', 'smtp_server'));
		...
		$param['active'] = true;
		$param['active_mce'] = is_plugin_active('tinymce-advanced/tinymce-advanced.php');
	...
	static public function updateSmtp($email = '', $password = '', $server, $port)
	{
		...
		$res = $mailer->send($adminEmail, __('Change mail server for newsletter', 'ms-plugins'), '
		<h4>' . __('Hello', 'ms-plugins') . '!</h4>
		<p>' . __('The site', 'ms-plugins') . ' ' . $blogName . ' ' . __(' has been changed SMTP settings for the MS-Subscriber plugin. Now the mailing will be from the address ', 'ms-plugins') . ' <b>' . $email . '</b>. ' . __('If you are reading this message, then everything is set up correctly.', 'ms-plugins') . '</p>
', array($blogName, $email));
	...
	static public function setSubscriber($email, $pageId)
	{
		...
		$subject = __('Subscribe to news confirmation', 'ms-plugins');
		$mess = __('Hello', 'ms-plugins') . '!<br>';
		$mess .= __('You or someone else indicated your e-mail to subscribe to the news site', 'ms-plugins');
		$mess .= ' <b>' . get_option('blogname') . '</b> ';
		$mess .= __('To confirm the subscription to the news, follow the link:', 'ms-plugins');
		...
		$mess .= __('If it was not you, then just ignore this letter.', 'ms-plugins');
	...
	static public function addUnsubscribeStr(&$str, $email)
	{
		...
		$str .= '<div>' . __('If you no longer want to receive news from our website, click', 'ms-plugins');
		$str .= ' ' . '<a href="' . get_home_url() . '?ms_unsubscribe=true&email=' . $email . '" target="_blank">';
		$str .= __('here', 'ms-plugins');
		$str .= '</a>';
		$str .= '</div>';
	}
	...
	static public function getSubscribersList()
	{
		...
		foreach ($subscribers as $subscriber) {
			...
			if ($subscriber->active > 0)
				$shtml .= __('Active', 'ms-plugins');
			else
				$shtml .= __('Non active', 'ms-plugins');
			$shtml .= '</td>';
			$shtml .= '</tr>';
		}
		$shtml .= '</table>';
	...
	static public function getTemplatesList()
	{
		...
		if (empty($list))
			return __('No templates found', 'ms-plugins');
