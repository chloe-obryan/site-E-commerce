...
function msweb_MSSubscriber_menu()
{
	if (isset($_POST['action']) && $_POST['action'] == 'heartbeat')
		return;
	add_submenu_page('ms-plugins', __('MS-Subscriber to news', 'ms-plugins'), 'MS-Subscriber', 'manage_options', \MS_WEB\MS_SUBSCRIBER\Main::PAGE_NAME, 'msweb_MSSubscriberPageOption', 'dashicons-editor-table');
}
...
{
	load_plugin_textdomain('ms-plugins', false, dirname(plugin_basename(__FILE__)) . '/languages/');
}
