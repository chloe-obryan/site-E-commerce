(function ($) {
  ...
  A.saveEmail = function () {
	...
	var emailInput = $('[ms-input][name="smtp-email"]');
	...
	if (!A.validateEmail(email)) {
	  emailInput.css({
		...
		if (A.validateEmail(this.innerText))
	...
	if (server == 'other') {
	  inpOtherServer = $('[name="smtp-server-other"]');
	  inpOtherServerValue = inpOtherServer.text();
	  inpOtherPort = $('[name="smtp-server-other-port"]');
	  inpOtherPortValue = inpOtherPort.text();

	  var empty = false, placeholder;

	  if (!inpOtherServerValue || (placeholder = inpOtherServer.attr('placeholder')) && inpOtherServerValue == placeholder) {
		A.setState(inpOtherServer, 'danger');
		A.addListenerOnSetState(inpOtherServer, 'default');
		empty = true;
	  }
	  if (!inpOtherPortValue || (placeholder = inpOtherPort.attr('placeholder')) && inpOtherPortValue == placeholder) {
		A.setState(inpOtherPort, 'danger');
		A.addListenerOnSetState(inpOtherPort, 'default');
		empty = true;
	  }
	...
	A.setLoader();
	var password = $('[ms-input][name="smtp-password"]').text();
  ...
  $(document).ready(function () {
	...
	$('[ms-input]').attr('contenteditable', true);
